...
# File
Three versions are stored in the different sub-repositories and the version is specified as the name of the repositories. In each repo, the predicted enhancer-gene links in each cell type are listed in different files, which is named as `cell_index.txt`. The name and detailed description of the cell type can be found at `ENCODE_cell_index.csv`. The description of the cell can be retrived by matching the `cell_index` from the file name and the `ID` column from the `ENCODE_cell_index.csv`.
